	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (name != null) {// Если в сессии имеется имя пользователя...
			...
			if (aUser != null && aUser.getSessionId().equals((String)request.getSession().getId())) {
				...
				response.sendRedirect(response.encodeRedirectURL("/labb8_war_exploded/"));// Перенаправить на главную страницу
				//-------------------------------------------------------------------------------------------------//
			} else {
		...
		/*else {
			response.addCookie(new Cookie("sessionId", null));
			response.sendRedirect(response.encodeRedirectURL("/labb8_war_exploded/login.do"));// Перенаправить пользователя на главное окно чата
		}
		*/
	}
}
